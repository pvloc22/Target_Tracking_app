...
import 'package:to_do_list/core/enums.dart';
...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  void _fetchData(LoadDataTask event, Emitter<HomeState> emit) async {
    ...
    try {
      emit(state.copyWith(postStatus: PostStatus.success, message: 'success', listTodayTasks: taskList));
    } catch (error) {
  ...
  void _fetchDataOfWeek(FetchDataTaskOfWeekEvent event, Emitter<HomeState> emit) async {
    ...
    emit(state.copyWith(postStatus: PostStatus.success, message: 'success', listTodayTasks: taskListOfWeek));
    // ignore: invalid_use_of_visible_for_testing_member
  }
  ...
  Future<List<Task>> searchItem(String keySearch) async {
    ...
    try {
      var result = taskList.where((task) =>
          task.title.toString().toLowerCase().contains(keySearch.toLowerCase()) ||
  ...
  void _searchTask(SearchTaskEvent event, Emitter<HomeState> emit) async {
    await searchItem(event.keySearch).then((value) => {
          ...
          if (value.isNotEmpty)
            {emit(state.copyWith(postStatus: PostStatus.success, message: 'success', listTodayTasks: List.from(value)))}
          else
  ...
  void _addTask(AddTaskEvent event, Emitter<HomeState> emit) async {
    taskList = List<Task>.from(state.listTodayTasks)..add(event.task);
